...
export default function ExpenseTab({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isCashAdvance = isAdvance || category === 'Cash Advance';
    const finalCategory = isCashAdvance ? 'Cash Advance' : category;
    ...
    try {
      const payload = {
        ...
        account: isCashAdvance ? 'Cash' : account,
        isAdvance: isCashAdvance,
        advancePerson: isCashAdvance ? advancePerson : '',
        advanceStatus: isCashAdvance ? advanceStatus : 'Pending'
      };
  ...
  // Global cash advances metrics
  const allTimeAdvances = expenses.filter(e => e.isAdvance || e.category === 'Cash Advance');
  const allTimeAdvancesTotal = allTimeAdvances.reduce((sum, item) => sum + (item.amount || 0), 0);
  const allTimePendingAdvances = allTimeAdvances.filter(e => (e.advanceStatus || 'Pending') === 'Pending');
  const allTimePendingTotal = allTimePendingAdvances.reduce((sum, item) => sum + (item.amount || 0), 0);
  const allTimeAdjustedAdvances = allTimeAdvances.filter(e => e.advanceStatus === 'Adjusted');
  const allTimeAdjustedTotal = allTimeAdjustedAdvances.reduce((sum, item) => sum + (item.amount || 0), 0);

  // Grouped by recipient/person
  const advancesByPerson = allTimeAdvances.reduce<Record<string, { total: number; pending: number; adjusted: number; count: number }>>((acc, item) => {
    const person = item.advancePerson?.trim() || 'General Materials';
    if (!acc[person]) {
      acc[person] = { total: 0, pending: 0, adjusted: 0, count: 0 };
    }
    const amt = item.amount || 0;
    acc[person].total += amt;
    acc[person].count += 1;
    if (item.advanceStatus === 'Adjusted') {
      acc[person].adjusted += amt;
    } else {
      acc[person].pending += amt;
    }
    return acc;
  }, {});

  // Cash Advances button handler: Filters table to show all advance amounts
  const handleCashAdvancesClick = () => {
    if (categoryFilter === 'Cash Advance') {
      setCategoryFilter('All');
      addToast('Reset filter to all expenses', 'info');
    } else {
      setCategoryFilter('Cash Advance');
      changeTimeFilter('all');
      setAccountFilter('All');
      setSearchQuery('');
      addToast(`Showing all Cash Advances (${allTimeAdvances.length} records, ${allTimeAdvancesTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })} SAR)`, 'info');
      setTimeout(() => {
        document.getElementById('expenses-table-card')?.scrollIntoView({ behavior: 'smooth' });
      }, 100);
    }
  };
  ...
  const handleExportCsv = () => {
    if (sortedList.length === 0) {
      addToast('No expense records found to export.', 'info');
      return;
    }
    const cleanMonth = activeTimeFilter === 'monthly' && selectedMonth ? selectedMonth.replace(/\s+/g, '_') : 'all_time';
    const filename = `care_elevator_expenses_${cleanMonth}_${new Date().toISOString().slice(0, 10)}.csv`;
    const success = exportExpensesToCsv(sortedList, filename);
    if (success) {
      addToast(`Successfully downloaded ${sortedList.length} expense records as CSV!`, 'success');
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col gap-5">
        ...
        {/* Breakdown Badges: Cash, Bank & Material Advances (Interactive Filters) */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-100">
          <button
            type="button"
            onClick={() => setAccountFilter(prev => prev === 'Cash' ? 'All' : 'Cash')}
            className={`p-3 rounded-2xl flex items-center justify-between transition cursor-pointer text-left select-none ${
              accountFilter === 'Cash'
                ? 'bg-emerald-100/90 border-2 border-emerald-500 shadow-sm ring-2 ring-emerald-400/20'
                : 'bg-slate-50 hover:bg-slate-100/80 border border-slate-200/70 hover:border-slate-300'
            }`}
            title={accountFilter === 'Cash' ? 'Click to reset filter' : 'Filter by My Cash Outflow'}
          >
            <span className="text-xs font-bold text-slate-600 flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500"></span>
              <span>My Cash Outflow:</span>
              {accountFilter === 'Cash' && (
                <span className="text-[9px] px-1.5 py-0.2 bg-emerald-600 text-white rounded font-extrabold uppercase">
                  Active
                </span>
              )}
            </span>
          ...
          <button
            type="button"
            onClick={() => setAccountFilter(prev => prev === 'Bank' ? 'All' : 'Bank')}
            className={`p-3 rounded-2xl flex items-center justify-between transition cursor-pointer text-left select-none ${
              accountFilter === 'Bank'
                ? 'bg-blue-100/90 border-2 border-blue-500 shadow-sm ring-2 ring-blue-400/20'
                : 'bg-slate-50 hover:bg-slate-100/80 border border-slate-200/70 hover:border-slate-300'
            }`}
            title={accountFilter === 'Bank' ? 'Click to reset filter' : 'Filter by Company Bank Outflow'}
          >
            <span className="text-xs font-bold text-slate-600 flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-blue-500"></span>
              <span>Company Bank:</span>
              {accountFilter === 'Bank' && (
                <span className="text-[9px] px-1.5 py-0.2 bg-blue-600 text-white rounded font-extrabold uppercase">
                  Active
                </span>
              )}
            </span>
          ...
          <button
            type="button"
            id="btn-cash-advances"
            onClick={handleCashAdvancesClick}
            className={`p-3 rounded-2xl flex items-center justify-between transition cursor-pointer text-left select-none group ${
              categoryFilter === 'Cash Advance'
                ? 'bg-amber-100/90 border-2 border-amber-500 shadow-md ring-2 ring-amber-400/30'
                : 'bg-amber-50/70 hover:bg-amber-100/80 border border-amber-200/80 hover:border-amber-300'
            }`}
            title={categoryFilter === 'Cash Advance' ? 'Click to show all expenses' : 'Click to display all Cash Advances'}
          >
            <span className="text-xs font-bold text-amber-900 flex items-center gap-1.5">
              ...
              {categoryFilter === 'Cash Advance' ? (
                <span className="text-[9px] px-1.5 py-0.2 bg-amber-600 text-white rounded font-extrabold uppercase tracking-wider">
                  Active
                </span>
              ) : (
                <span className="text-[9px] px-1.5 py-0.2 bg-amber-200/80 text-amber-900 rounded font-semibold hidden sm:inline">
                  Click to View All
                </span>
              )}
            </span>
            <span className="text-sm font-black text-amber-900 flex items-center gap-1">
              ...
              {(categoryFilter === 'Cash Advance' ? currentTotal : allTimeAdvancesTotal).toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </span>
          </button>
        </div>
      </div>
      ...
      <div id="expenses-table-card" className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-100">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200/80 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
              title="Download filtered expenses as CSV file"
            >
              <Download className="w-3.5 h-3.5" />
              <span>Export CSV</span>
            </button>
            {allTimeAdvances.length > 0 && (
              <button
                type="button"
                onClick={() => setShowAdvanceModal(true)}
                className="px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200/80 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-sm cursor-pointer"
                title="View Cash Advances breakdown by recipient and status"
              >
                <Coins className="w-3.5 h-3.5 text-amber-600" />
                <span>Advances Summary</span>
              </button>
            )}
          </div>
        ...
        {/* Active Cash Advance Banner Notification with Quick Summary & Actions */}
        {categoryFilter === 'Cash Advance' && (
          <div className="mb-5 p-4 bg-amber-50/90 border border-amber-300/80 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs shadow-xs">
            <div className="flex items-center gap-3 text-amber-950 font-bold">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm font-black text-amber-900">All Cash Advances</span>
                  <span className="px-2 py-0.5 bg-amber-200 text-amber-900 rounded-full font-extrabold text-[10px]">
                    {filteredList.length} Records
                  </span>
                </div>
                <div className="text-[11px] text-amber-800 font-medium mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5">
                  <span>
                    Total Amount: <strong className="text-amber-950">{currentTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })} SAR</strong>
                  </span>
                  <span>
                    Pending: <strong className="text-amber-900">{allTimePendingTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })} SAR</strong>
                  </span>
                  <span>
                    Adjusted: <strong className="text-emerald-800">{allTimeAdjustedTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })} SAR</strong>
                  </span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                type="button"
                onClick={() => setShowAdvanceModal(true)}
                className="px-3.5 py-2 bg-white hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-xl font-bold flex items-center gap-1.5 transition cursor-pointer shadow-xs"
              >
              ...
              <button
                ...
                className="px-3.5 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-xl font-bold flex items-center gap-1.5 transition cursor-pointer shadow-xs"
              >
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {sortedList.length === 0 ? (
                ...
                sortedList.map((item, index) => {
                  ...
                  return (
                    <tr
                      key={item.id ? `exp-${item.id}` : `exp-${item.rowId}-${index}`}
      ...
      {/* Cash Advances Overview & Breakdown Modal */}
      <AnimatePresence>
        {showAdvanceModal && (
          <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white border border-slate-200 rounded-[2rem] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden relative"
            >
              {/* Header */}
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-black text-slate-800 uppercase tracking-tight">Cash Advances Directory</h3>
                    <p className="text-xs text-slate-500">Overview of all material and staff cash advances</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="p-6 overflow-y-auto space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="p-4 bg-amber-50/70 border border-amber-200/80 rounded-2xl">
                    <span className="text-xs font-bold text-amber-800 uppercase tracking-wider block">Total Advances Given</span>
                    <span className="text-xl font-black text-amber-950 flex items-center gap-1 mt-1">
                      <SaudiRiyalIcon className="w-4 h-4 text-amber-800" />
                      {allTimeAdvancesTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </span>
                    <span className="text-[11px] text-amber-700 mt-1 block font-semibold">{allTimeAdvances.length} records in total</span>
                  </div>

                  <div className="p-4 bg-rose-50/70 border border-rose-200/80 rounded-2xl">
                    <span className="text-xs font-bold text-rose-800 uppercase tracking-wider block">Pending / Unadjusted</span>
                    <span className="text-xl font-black text-rose-950 flex items-center gap-1 mt-1">
                      <SaudiRiyalIcon className="w-4 h-4 text-rose-800" />
                      {allTimePendingTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </span>
                    <span className="text-[11px] text-rose-700 mt-1 block font-semibold">{allTimePendingAdvances.length} pending records</span>
                  </div>

                  <div className="p-4 bg-emerald-50/70 border border-emerald-200/80 rounded-2xl">
                    <span className="text-xs font-bold text-emerald-800 uppercase tracking-wider block">Adjusted / Settled</span>
                    <span className="text-xl font-black text-emerald-950 flex items-center gap-1 mt-1">
                      <SaudiRiyalIcon className="w-4 h-4 text-emerald-800" />
                      {allTimeAdjustedTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </span>
                    <span className="text-[11px] text-emerald-700 mt-1 block font-semibold">{allTimeAdjustedAdvances.length} adjusted records</span>
                  </div>
                </div>
                ...
                <div>
                  <h4 className="text-xs font-black text-slate-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                    ...
                    <span>Advances by Recipient / Staff</span>
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {Object.entries(advancesByPerson).map(([person, stat]) => (
                      <div key={person} className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl flex flex-col justify-between">
                        <div className="flex items-center justify-between">
                          <span className="font-extrabold text-sm text-slate-800">{person}</span>
                        ...
                        <div className="mt-3 pt-2 border-t border-slate-200/60 flex items-center justify-between text-xs">
                          ...
                          <span className="font-black text-slate-900 flex items-center gap-0.5">
                            <SaudiRiyalIcon className="w-3 h-3 text-slate-700" />
                            {stat.total.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                          </span>
                        </div>
                        <div className="mt-1 flex items-center justify-between text-[11px]">
                          <span className="text-rose-600 font-medium">Pending: {stat.pending.toLocaleString()}</span>
                          <span className="text-emerald-700 font-medium">Adjusted: {stat.adjusted.toLocaleString()}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* All Advance Records List */}
                <div>
                  <h4 className="text-xs font-black text-slate-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                    <Coins className="w-4 h-4 text-amber-600" />
                    <span>All Advance Records ({allTimeAdvances.length})</span>
                  </h4>
                  {allTimeAdvances.length === 0 ? (
                    <p className="text-xs text-slate-500 text-center py-6">No cash advances recorded yet.</p>
                  ) : (
                    <div className="border border-slate-200 rounded-xl overflow-x-auto max-h-60 overflow-y-auto">
                      <table className="w-full text-left text-xs whitespace-nowrap">
                        <thead className="bg-slate-100 text-slate-700 text-[10px] font-bold uppercase sticky top-0">
                          <tr>
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {allTimeAdvances.map((adv, idx) => (
                            <tr key={adv.rowId || adv.id || idx} className="hover:bg-slate-50/80">
                              <td className="p-3 font-semibold text-slate-700">{adv.date}</td>
                              <td className="p-3 font-bold text-slate-900">{adv.advancePerson || 'General Materials'}</td>
                              <td className="p-3 text-slate-600 max-w-xs truncate">{adv.description || adv.liftNo || 'Material advance'}</td>
                              <td className="p-3 text-slate-500">{adv.invoice || 'N/A'}</td>
                              <td className="p-3">
                                <span className={`px-2 py-0.5 rounded-md font-bold text-[10px] ${
                                  adv.advanceStatus === 'Adjusted'
                                    ? 'bg-emerald-100 text-emerald-800'
                                    : 'bg-amber-100 text-amber-900'
                                }`}>
                                  {adv.advanceStatus || 'Pending'}
                                </span>
                              </td>
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                <button
                  ...
                  onClick={() => {
                    ...
                    if (categoryFilter !== 'Cash Advance') {
                      handleCashAdvancesClick();
                    }
                  }}
                  className="px-4 py-2.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-bold transition cursor-pointer flex items-center gap-1.5 shadow-xs"
                >
                  <Coins className="w-3.5 h-3.5" />
                  <span>Filter Main Table to All Advances</span>
                </button>
                <button
                  type="button"
                  onClick={() => setShowAdvanceModal(false)}
